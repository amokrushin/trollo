...
const { keys, castArray, toPath, intersection, without, chain, isUndefined, isObjectLike, first } = require('lodash');
...
class EntityCreate {
    constructor(Model, data, params = {}) {
        ...
        this._sql = squel.update().table(this._Model.tableName);
    }
    ...
    _returningPrimaryKey() {
        ...
        const primaryKey = castArray(schema.primaryKey || 'id');

        primaryKey.forEach(f => {
            this._sql.returning(`${prefix}${snakeCase(f)}`, camelCase(f));
        });
    }

    _buildFilter() {
        const { schema } = this._Model;
        const primaryKey = castArray(schema.primaryKey || 'id');

        this._query = {
            filter: primaryKey.reduce((acc, key) => {
                return set(acc, key, { $eq: get(this._data, key, null) });
            }, {}),
        };
    }
    ...
    _setFields() {
        const { schema, fieldsNamesByType, fieldSchemaByPath } = this._Model;

        const primaryKey = castArray(schema.primaryKey || 'id');
        const dataKeys = without(keys(this._data), ...fieldsNamesByType.virtual, ...primaryKey);
